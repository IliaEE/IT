...
Expo (SDK 57) app — one codebase for iOS, Android and web. Three on-device photo utilities,
no server, no AI: **Convert** (HEIC/JPG/PNG + photo→PDF), **Image Size** (resize, batch, fit-to-ratio,
DPI), **Compress** (quality slider with live size). Product context lives in `docs/`:
...
(popularity above the floor of 5 in the target markets). Section 5.3 there is the source of truth for
